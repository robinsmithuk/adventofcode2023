...
with open(path, newline='') as csvfile:
    ...
    for row in rowreader:
        ...
        #Rock
        if row[0] == o_rock and row[1] == p_draw:
            player_result = draw + rock_point
        
        if row[0] == o_rock and row[1] == p_win:
            player_result = won + paper_point

        if row[0] == o_rock and row[1] == p_lose:
            player_result = lost + scissor_point

        #Paper
        if row[0] == o_paper and row[1] == p_lose:
            player_result = lost + rock_point
        
        if row[0] == o_paper and row[1] == p_draw:
            player_result = draw + paper_point

        if row[0] == o_paper and row[1] == p_win:
            player_result = won + scissor_point

        #Scissors
        if row[0] == o_scissors and row[1] == p_win:
            player_result = won + rock_point
        
        if row[0] == o_scissors and row[1] == p_lose:
            player_result = lost + paper_point

        if row[0] == o_scissors and row[1] == p_draw:
